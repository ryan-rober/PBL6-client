...
const BookingScreen = () => {
  const [isClickInfor, setIsClickInfor] = useState('');
  ...
  useEffect(() => {
    if (!clicksortprice) {
      setDataSort(data?.sort((a, b) => a.price - b.price));
    } else {
      setDataSort(data?.sort((a, b) => b.price - a.price));
    }
    if (!clicksorttime) {
      setDataSort(data?.sort((a, b) => convertTime(a.timeStart) - convertTime(b.timeStart)));
    } else {
      setDataSort(data?.sort((a, b) => convertTime(b.timeStart) - convertTime(a.timeStart)));
    }
  }, [dataSort, data, clicksortprice, clicksorttime]);

  const onClickInfor = (id) => {
    if (isClickInfor === '') {
      setIsClickInfor(id);
    }
    if (isClickInfor === id) {
          ...
          <div className="flex">
            ...
            <div className="w-1/5 h-20 rounded-lg m-6 item flex bg_red">
              ...
              <div className="my-auto w-32 cursor-pointer">
                <p className="text-white mb-1" onClick={() => onSubmitFindRoute()}>
                  Tìm chuyến
                </p>
              </div>
            </div>
          </div>
          <h1 className="text-[16px] ml-6 py-4">
            Đặt vé xe đi {endpoint.label} từ {startpoint.label} chất lượng cao và giá vé ưu đãi nhất: {data?.length} chuyến
          </h1>
          <div className="flex ml-6 text-[15px]">
            <p className="mr-16 font-bold">Sắp xếp theo :</p>
            <p className="mr-16 cursor-pointer" onClick={() => { handleSortTimeUp(); }}>
              Giờ đi sớm nhất
            </p>
            <p className="mr-16 cursor-pointer" onClick={() => { handleSortTimeDown(); }}>
              Giờ đi muộn nhất
            </p>
            <p className="mr-16 cursor-pointer" onClick={() => { handleSortPriceUp(); }}>
              Giá tăng dần
            </p>
            <p className="cursor-pointer" onClick={() => { handleSortPriceDown(); }}>
              Giá giảm dần
            </p>
          </div>
              ...
              <div className="main flex ">
                <div className="flex w-1/5 mr-6">
                  ...
                  <p className="font-bold text-lg">
                    {item.timeStart}
                    <br /> | <br /> {convertTimeToNumber(item.timeStart, item.timeStations.length)}
                  </p>
                </div>
                <div className="w-1/5 block mr-14">
                  <p className="font-bold text-[17px] mb-0">{item.nameVehicle}</p>
                  <p
                    className="text-[12px] underline text-sky-500 flex cursor-pointer item_"
                    key={index}
                    onClick={() => onClickInfor(item.idTrip)}
                  >
                    Thông tin chi tiết
                ...
                <div className="w-1/5 block ">
                  <p className="font-bold text-[15px] mb-0 text-[#4457FF] pl-[15px]">
                    {formatCurrency(item.price)}VND
                    {!(isClickBook === item.idTrip) ? (
                      <>
                        <button className="bt-detail" onClick={() => onClickBook(item.idTrip)}>
                          Chọn tuyến
                        </button>
                      </>
                    ) : (
                      <>
                        <button className="bt-detail-close" onClick={() => onClickBook(item.idTrip)}>
                          Đóng
                        </button>
                      </>
                    )}
