...
import { getListLocation, findTrips, getProfile } from "@apis";
...
const BookingScreen = () => {
	...
	useEffect(() => {
		...
		setStartPoint(
			JSON.parse(
				getLocalStorage(STORAGE.startpoint) ||
					'{"label":"Đà Nẵng","value":1}'
			)
		);
		setEndPoint(
			JSON.parse(
				getLocalStorage(STORAGE.endpoint) ||
					'{"label":"Bình Định","value":4}'
			)
		);
		const my_date = new Date(
			JSON.stringify(getLocalStorage(STORAGE.date)).slice(3, 13)
		);
		setDate(my_date);
	...
	useEffect(() => {
		...
		if (!clicksorttime) {
			setDataSort(
				data?.sort(
					(a: any, b: any) =>
						convertTime(a.timeStart) - convertTime(b.timeStart)
				)
			);
		} else {
			setDataSort(
				data?.sort(
					(a: any, b: any) =>
						convertTime(b.timeStart) - convertTime(a.timeStart)
				)
			);
		}
			...
			<Wrapper id="intro" className="bg-white">
				...
				<div className="w-3/5 bg-white  container rounded-lg block">
					<div className="flex">
						<div className="w-1/5 h-20 rounded-lg m-6 item flex">
							...
							<div className="my-auto w-32">
								<p className="text-sky-600 mb-1 ml-[-8px]">
									Điểm đi
								</p>
								<Select
									id="country"
					...
					<h1 className="text-[16px] ml-6 py-4">
						Đặt vé xe đi {endpoint.label} từ {startpoint.label} chất
						lượng cao và giá vé ưu đãi nhất: {data?.length} chuyến
					</h1>
							...
							<div className="main flex ">
								<div className="flex w-1/5 mr-6">
									<img
										...
										src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQMmcVyV_1ANkrz2-grItqC5mmWqlCw19FDKw&usqp=CAU"
										alt=""
									/>
										...
										{!(isClickInfor === item.idTrip) ? (
											<>
												<img
													...
													src="https://icon-library.com/images/arrow-down-icon-png/arrow-down-icon-png-3.jpg"
													alt=""
												/>
											</>
										) : (
											<>
												<img
													className="w-[14px] ml-1 rotate-180"
													src="https://icon-library.com/images/arrow-down-icon-png/arrow-down-icon-png-3.jpg"
													alt=""
												/>
											</>
