...
import {ApiHistoryBooking, ApiRefund} from '@apis'
...
const Wrapper = styled.div`
  ...
  .payment{
    ...
    height: 30px;
    text-align: center;
    background: #af0303;//rgb(4, 69, 140);
...
const Current = () => {
  ...
  const listPayment = dataSort.filter(
  ...
  const listCancel = dataSort.filter((item) => item.historyBooking.payment.paymentMethods.status === "Success" 
  && new Date().valueOf() - Date.parse(item.historyBooking.dateStart) < 0)
  ...
  return (
    <Wrapper>
      <div className="block">
        ...
        <>
        <div>
         {Array.isArray(listPayment) && listPayment.map((item : any, index :any) => (
          <div>
      ...
      {listCancel.length ? 
        <>
        <div>
         {Array.isArray(listCancel) && listCancel.map((item : any, index :any) => (
          <div>
