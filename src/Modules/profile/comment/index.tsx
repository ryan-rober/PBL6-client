    ...
    <Wrapper>
      {listRating.length ? 
        ...
        {listRating.map((item : any) => (
         <div className='h-[12rem] w-[100%] flex mb-2 border-b'>
            ...
            <div className='block mx-6 my-4'>
               ...
               <p className='text-zinc-400 text-xs mb-1'>Chuyến : {item.depart} - {item.arrive} </p>
               <p className='text-zinc-400 text-xs mb-1'>Hãng xe : {item.agencyName}</p>
               <p className='text-zinc-400 text-xs'>Xe :     {item.vehicleName}</p>
            </div>
         </div>
      ))}
        </div>
