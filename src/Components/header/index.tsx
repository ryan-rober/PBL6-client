...
const Header: FC<Prop> = ({ setIsShow }) => {
	...
	const username = token
		? (JSON?.parse(getLocalStorage(STORAGE?.PROFILE_USER) || "{}")).name
		: null;
		...
		<Menu>
			{role === USER_ROLE.COMPANY_ADMIN && (
				<>
					<Menu.Item
						key="0"
						onClick={() => window.location.replace(USER_URL)}
					>
						<HomeOutlined />
						<span>&nbsp;User page</span>
					</Menu.Item>
			...
			<Menu.Item key="1">
				<img
					src="https://storage.googleapis.com/fe-production/images/Auth/account-circle.svg"
					alt=""
				/>
				<Link to="./profile">
			...
			<Menu.Item key="2" onClick={handleLogout}>
				<img
					src="https://storage.googleapis.com/fe-production/images/Auth/logout.svg"
					alt=""
				/>
						...
						<div className="row g-3">
							...
							<div className="col user-info" role="presentation">
								<button className="bg-sky-700 w-full flex font-semibold h-8 items-center justify-center  px-3 rounded-md text-white text-sm  fol">
									<img
										src="https://storage.googleapis.com/fe-production/images/Auth/account-circle-fill.svg"
										alt=""
									/>
									{isLoggedIn ? (
										<>
