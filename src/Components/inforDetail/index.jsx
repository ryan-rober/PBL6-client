...
const InforDetail = ({ isClickInfor, item }) => {
  ...
  return (
    <Wrapper className={`${isClickInfor === item.idTrip ? "block" : "hidden"}`}>
      <div className="mt-4 ml-20 ">
        <ul className="flex mb-0">
          {list.map((item, index) => {
            ...
            return (
              <li
                key={index}
                className={`dark:text-slate-700 mr-14 text-base  ${classStyles2}`}
              >
                <div
        ...
        <div>
          {list[0].isActive && <Image />}
          {list[1].isActive && <Utilites />}
          {list[2].isActive && <Point item={item} />}
          {list[3].isActive && <Policy />}
          {list[4].isActive && <Evaluate item={item} />}
        </div>
      </div>
    </Wrapper>
  );
