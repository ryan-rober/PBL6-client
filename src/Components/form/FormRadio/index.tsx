...
const FormRadio = ({ t, label, name, rules, defaultValue = '', wrapperProps, options = [], children, ...rest }: any) => {
  ...
  return (
    ...
    >
      <Radio.Group {...rest} onChange={onChange} value={value}>
        {children || options.map((opt: any) => <Radio key={opt.value} value={opt.value}>{t(opt.label)}</Radio>)}
      </Radio.Group>
    </WrapperFormItem>
  )
}
