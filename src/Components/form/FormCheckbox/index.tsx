...
const FormCheckbox = ({ t, label, name, rules, defaultValue = '', wrapperProps, options = [], ...rest }: any) => {
  ...
  return (
    ...
    >
      <Checkbox.Group {...rest} onChange={onChange} value={value}>
        {options.map((opt: any) => <Checkbox key={opt.value} value={opt.value}>{t(opt.label)}</Checkbox>)}
      </Checkbox.Group>
    </WrapperFormItem>
  )
}
